...
| **Ce que tenterait un exploiteur** | (a) marteler pour attaquer plus vite que le frame data ne l'autorise ; (b) réclamer directement le 4e coup pour obtenir un knockdown à la demande ; (c) attaquer depuis un état qui l'interdit (étourdi, au sol) ; (d) antidater `clientTime` pour décaler la fenêtre active du coup et se ménager un contact hors phase. |
